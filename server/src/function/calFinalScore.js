...
module.exports = async () => {
  ...
  const groupedData = Object.values(
    score.reduce((acc, { studentID, semester, average, name, classID }) => {
      if (!acc[studentID]) {
        acc[studentID] = {
          studentID,
          name,
  ...
  const result = groupedData.map(
    ({ studentID, name, classID, firstSemester, secondSemester }) => ({
      studentID,
      name,
      classID,
      ...
      FinalScore: parseFloat(
        ((firstSemester + 2 * secondSemester) / 3).toFixed(2)
      ),
    })
  );
